...
type RecurrenceConfig struct {
	...
	MaxPerProject int `mapstructure:"max_per_project" default:"10"`
	...
	MinProjectFetchLimit int `mapstructure:"min_project_fetch_limit" default:"5"`
...
func registerRecurrenceFlags(v *viper.Viper, flags *pflag.FlagSet) error {
	...
	err = config.BindConfigFlagWithShort(
		v, flags, "recurrence.max_retries", "max-retries", "r", 3,
		"Maximum retries for a reminder", flags.IntP)
	if err != nil {
		return err
	}
	...
	err = config.BindConfigFlag(
		...
		"Maximum number of reminders per project in a batch", flags.Int32)
	if err != nil {
		return err
	}

	err = config.BindConfigFlag(
		v, flags, "recurrence.min_project_fetch_limit", "min-project-fetch-limit", 5,
		"Minimum No. of projects to fetch in an iteration", flags.Int32)
	if err != nil {
		return err
	}
